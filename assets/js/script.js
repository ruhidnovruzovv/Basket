...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const favvHeart = document.querySelector(".favv");
  ...
  const updateHearts = (wishlist) => {
    const hearts = document.querySelectorAll(".fav");
    hearts.forEach((heart, index) => {
      const productId = products[index].id;
      const isInWishlist = wishlist.some((item) => item.id === productId);
      heart.classList.toggle("fa-solid", isInWishlist);
      heart.classList.toggle("fa-regular", !isInWishlist);
    });
  };

  const updateBasketCount = () => {
    if (userStatus && currentUser && currentUser.basket) {
      const totalCount = currentUser.basket.reduce((acc, product) => acc + product.count, 0);
  ...
  if (userStatus && currentUser) {
    ...
    updateHearts(currentUser.wishlist);
    updateBasketCount();
  }
  ...
  const toggleBasket = (product) => {
    ...
    updateBasketCount(); 
    showToast("Məhsul səbətə əlavə olundu.", "success");
    return productIndex === -1;
  };
  ...
  products.forEach((product) => {
    ...
    heart.className = "fa-regular fa-heart fav";
    ...
    heart.addEventListener("click", () => {
      ...
      if (userStatus) {
        heart.classList.toggle("fa-solid", wasAdded);
      }
    });
    ...
    count.textContent = `Reviews (${product.rating.count})`;
    ...
    card.append(rating, count, buyButton);

    const cardContainer = document.querySelector(".card-container");
    cardContainer.appendChild(card);
  });

  favvHeart.addEventListener("click", () => {
    if (userStatus) {
