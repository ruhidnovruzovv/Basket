document.addEventListener("DOMContentLoaded", () => {
    ...
    const renderBasket = () => {
      ...
      if (!currentUser || !currentUser.basket || currentUser.basket.length === 0) {
      ...
      basket.forEach((product, index) => {
        ...
        basketItem.querySelector(".increment").addEventListener("click", () => {
          ...
          itemPriceElement.textContent = (product.price * basket[index].count).toFixed(2);  
          renderBasket();  
        });
      ...
      const total = basket.reduce((acc, product) => acc + product.price * product.count, 0);
